...
class GoogleMapsResources(Resource):
    ...
    def get(self):
        ...
        Returns:
            ...
            {
                "adress": "Jl. Sukun No. 23, Malang, Sukun"
            }
        """
        ...
        params = "?latlng={},{}&key={}".format(
            args['lat'], args['lng'], google_maps_api_key)

        url = "{host}{params}".format(host=google_maps_host, params=params)

        latlng = "{lat},{lng}".format(lat=args['lat'], lng=args['lng'])

        request = requests.get(google_maps_host, params={
                               'latlng': latlng, 'key': google_maps_api_key})
